...
#define GET_BA_VALUE(type, values, start_idx)  Pipeline::GetBarycentricValue<type>(values, start_idx, alpha, beta, gamma)
...
public: 
	...
	static void RunFragmentStage(int start_vertex_idx); // »­Èý½ÇÐÎ

	template <typename T> static T GetBarycentricValue(const T* values, int start_idx, float alpha, float beta, float gamma)
	{
		return values[start_idx] * alpha + values[start_idx + 1] * beta + values[start_idx + 2] * gamma;
	}
private:
